...
| Signal | Claude Code | Codex CLI | Cline | Hermes |
| --- | --- | --- | --- | --- |
| Generated per-role wrapper present | the plugin package's `agents/*.md` (or a project-local `.claude/agents/<role-id>.md` override) | `provider/codex-agents/agents-*.toml`, synced to `~/.codex/agents/` | [`cline-plugins/cline-agents/agents/<role-id>.md`](https://github.com/deagy/cadre/tree/main/cline-plugins/cline-agents/agents) — 159 generated Cline SDK agent presets, one per catalog role | [`hermes-plugins/skills/cadre/<domain>/<role-id>/SKILL.md`](https://github.com/deagy/cadre/tree/main/hermes-plugins/skills/cadre) — 172 generated skills (159 roles, 12 workflows, `cadre-orchestrator`), installed under `~/.hermes/skills/cadre/` |
| Project config directory | `.claude/` (`.claude/agents/`, `.claude/skills/`) | `.codex/` (`.codex/agents/`, `.codex/config.toml`) | `.clinerules/` (one general pointer file, not per-role) | `~/.hermes/` (`config.yaml`, `skills/`); a trusted project (`hermes skills trust`) also loads `.hermes/skills/` at its git root |
| Subagent-dispatch tool name in the session | `Agent`/`Task` tool referencing a named subagent type | `spawn_agent` tool with a generic `agent_type` argument | `start_subagent` (its `preset` argument names a role directly, e.g. `preset: "security-reviewer"`) and `dispatch_selected_roles` (runs `cadre select`, then `start_subagent`s every selected primary/reviewer role), both registered by [`cline-plugins/cline-agents/`](https://github.com/deagy/cadre/tree/main/cline-plugins/cline-agents) (`index.ts`); the sibling [`cline-plugins/cline/`](https://github.com/deagy/cadre/tree/main/cline-plugins/cline) plugin registers only `agents_select`, which plans and never dispatches | `delegate_task` (`goal` + `context`); the role's contract reaches the child as `context` after `skill_view` loads its skill, and `cadre-orchestrator` says how |
| Distinguishing environment/config signal | `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` env var gates Agent Teams | `config.toml` `[mcp_servers.*]` / `[agents]` blocks | `~/.cline/data/teams/[team-name]/` if team mode has run | `HERMES_HOME` (default `~/.hermes`); `skill_view` and `delegate_task` in the tool list; `TERMINAL_CWD` decides where the terminal starts (the home directory unless set) |

## What that implies

| Property | Claude Code | Codex CLI | Cline | Hermes |
| --- | --- | --- | --- | --- |
| Generated wrapper exists for this repo's roles | Yes | Yes | Yes — `cline-plugins/cline-agents/agents/<role-id>.md`, drift-guarded byte-for-byte in CI | Yes — `hermes-plugins/skills/cadre/`, drift-guarded byte-for-byte by `internal/generators/hermes_port_test.go` |
| How a role is named for dispatch | `agents:<role-id>` (plugin-installed) or bare `<role-id>` (project-local override) | `.codex/agents/<role-id>.toml` (project) or `~/.codex/agents/agents-<role-id>.toml` (global) | `start_subagent`'s `preset` argument = the role id (each preset's frontmatter `name:` matches its `agents/<role-id>.md` filename) | The skill name is the role id (`skill_view(name="security-reviewer")`); `delegate_task` itself takes no role name, only the contract text |
| Can the model-visible dispatch tool select a *named* custom role directly? | Yes | **No** — `spawn_agent` only accepts a generic `agent_type`; there is no parameter for a named `.codex/agents/` entry (tracked upstream as openai/codex#15250 and related issues) | Yes — `start_subagent(preset: "<role-id>")` in the `cline-agents` plugin; the sibling `cline` plugin's `agents_select` plans only and cannot dispatch | No — `delegate_task` has no preset or agent-type argument; the role is whatever contract the caller passes as `context` |
| Workaround when named dispatch isn't supported | Not applicable (natively supported) | Preferred: register this repo's MCP dispatch server (`cadre mcp-dispatch-server`) and call `dispatch_secure_cloud_role`. Fallback: read the target `.toml` file's `developer_instructions`/`model` and inject them manually into `spawn_agent` | Not applicable (natively supported via `start_subagent`/`dispatch_selected_roles`). Presets take their provider/model from `CLINE_AGENTS_PROVIDER_ID` and `CLINE_AGENTS_MODEL_<TIER>`/`_DEFAULT` when set, and otherwise inherit the dispatching session's own provider/model as a pair; only when neither resolves does dispatch fail closed | `skill_view` the role skill and pass its contract verbatim as `delegate_task` `context`, together with the working directory from `pwd` (the `cadre-orchestrator` skill's Dispatch Protocol, steps 0 and 3) |
| Peer-to-peer teammate messaging (`communication_mode: "peer"`) | Supported, but gated: requires `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`; falls back to `orchestrator-relayed` if unset | Not supported — no peer messaging or shared task list; coordination is entirely orchestrator-centric | Best-effort only — team mode exists (`/team`, `cline --team-name`) but the *coordinator's own model* decides teammate composition and messaging; not guaranteed the way Claude Code's gated Agent Teams are | Not supported — children report to the parent only |
| Nested teams (a teammate spawning its own teammates) | Not supported — runner limitation | Not applicable (no team primitive at all) | Not applicable in the same sense; team state persists under `~/.cline/data/teams/[team-name]/` but there is no per-role teammate naming to nest | Not supported — `delegate_task` is removed from a child's tool set (no recursive delegation) |
| Team size guidance | 3–5 teammates, disjoint file ownership per teammate | Not applicable | Not applicable | Not applicable |
| Concurrency bound | Not separately documented beyond the Agent tool's own session limits | `agents.max_concurrent_threads_per_session` (`[agents]` block, native `spawn_agent`) or `MAX_CONCURRENT_CHILDREN` (this repo's MCP dispatch server) | Not separately documented | `delegation.max_concurrent_children` in `~/.hermes/config.yaml` (default 10) |
| Can roles run against a self-hosted model? | Not through this repo's tooling — Claude Code has no custom-provider mechanism | Yes: a `[model_providers.*]` block in a Codex config profile, selected by `runners.codex_profile`, with `runners.local_model_<tier>` supplying the per-tier model. See `docs/INSTALL.md` | Yes, but through Cline's own provider config: `CLINE_AGENTS_PROVIDER_ID` + `CLINE_AGENTS_MODEL_<TIER>` | Yes — `model.provider: custom` with `model.base_url` in `~/.hermes/config.yaml` (any OpenAI-compatible endpoint); the run that validated this tree used a local vLLM |
...
- **Hermes** dispatches through `delegate_task`, which carries no role
  name: the `cadre-orchestrator` skill is what turns a role id into a
  dispatch, by loading the role skill with `skill_view` and passing its
  contract as the child's `context` with the working directory from `pwd`.
  Launch with `TERMINAL_CWD="$PWD"`: the terminal otherwise starts in the
  home directory, and the skill's instruction to stop and ask rather than
  work there is advisory, not enforced (a probe without the variable still
  dispatched a read-only child into `~`). Children cannot delegate further and cannot message each other, so
  every team recipe runs `orchestrator-relayed`. A `pre_tool_call` hook
  (`hooks:` in `cli-config.yaml`, or a plugin) can refuse a tool call, so a
  policy is enforceable there; nothing runs on the user's prompt before the
  model sees it. Worktree isolation for children is opt-in
  (`delegation.worktree_isolation`, default off).
