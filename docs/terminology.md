...
| Route | One entry in [`roster/orchestration/routing.yaml`](../roster/orchestration/routing.yaml)'s `routes[]` array: a set of file-path patterns and/or keywords mapped to primary/reviewer/support roles and required quality gates. A task matches zero, one, or several routes; `cadre select` reports the matches as `matched_routes`, and why each one matched (the keywords, keyword groups, and path patterns that fired) as `matched_route_reasons`. See the [sample selection output](sample-selection-output.md) for a worked example. |
