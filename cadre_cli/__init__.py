...
def main(argv: list[str] | None = None) -> int:
    ...
    # `generate-plugin` fails closed from this channel regardless of install
    # kind, bundled or editable. The Go generator
    # (internal/generators/plugin_generation.go, reached via ./bin/cadre) is
    # the sole producer of the committed plugin/ tree, and it is the only one
    # that emits the hardened bin/cadre shim -- binary resolution, mandatory
    # checksum verification, sidecar-verified cache, permission-gated exec.
    # roster/orchestration/src/generate_global_plugin.py's generate_bin_wrapper()
    # still emits the pre-hardening shim, so dispatching here from an editable
    # checkout install silently regenerated a plugin whose launcher had none of
    # those controls. One generator, or they drift again.
    if command == "generate-plugin":
        print(
            "cadre generate-plugin: run ./bin/cadre generate-plugin from a "
            "repository checkout instead. The packaged plugin is generated by "
            "the Go implementation, which is the only generator that emits the "
            "hardened bin/cadre launcher; this Python channel would produce a "
            "launcher without binary verification.",
            file=sys.stderr,
        )
        return 2
