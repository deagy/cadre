...
func TestConsistentHashRingBasic(t *testing.T) {
	...
	ring := NewConsistentHashRing(shards, 150)

	// Same key should always map to same shard
	shard1 := ring.GetShard("key-1")
	shard2 := ring.GetShard("key-1")
	if shard1 != shard2 {
		t.Errorf("Consistent hashing failed: %s != %s", shard1, shard2)
	}

	// Verify shard is one of the registered shards
	found := false
	...
	if !found {
		t.Errorf("Shard '%s' not in registered shards", shard1)
	}
}
...
func TestConsistentHashRingDistribution(t *testing.T) {
	...
	for _, shard := range shards {
		if distribution[shard] == 0 {
			t.Logf("Warning: shard '%s' has no keys", shard)
		}
	}
}

func TestConsistentHashRingAddShard(t *testing.T) {
	shards := []string{"shard-0", "shard-1"}
	ring := NewConsistentHashRing(shards, 150)

	// Get shard assignment before adding
	key := "test-key"
	shardBefore := ring.GetShard(key)

	// Add new shard
	ring.AddShard("shard-2")

	// Shard assignment might change (expected for consistent hashing)
	shardAfter := ring.GetShard(key)
	t.Logf("Key '%s' mapped to '%s' before, '%s' after adding shard", key, shardBefore, shardAfter)

	// Verify new shard is in ring
	allShards := ring.GetAllShards()
	if len(allShards) != 3 {
		t.Errorf("Expected 3 shards, got %d", len(allShards))
	}
}

func TestConsistentHashRingRemoveShard(t *testing.T) {
	shards := []string{"shard-0", "shard-1", "shard-2"}
	ring := NewConsistentHashRing(shards, 150)
	...
	allShards := ring.GetAllShards()
	if len(allShards) != 2 {
		t.Errorf("Expected 2 shards after removal, got %d", len(allShards))
	}

	// Key should still map to remaining shards
	key := "test-key"
	shard := ring.GetShard(key)
	if shard == "shard-1" {
		t.Errorf("Key mapped to removed shard")
	}
}

func TestConsistentHashRingDuplicateAdd(t *testing.T) {
	shards := []string{"shard-0", "shard-1"}
	ring := NewConsistentHashRing(shards, 150)

	// Try adding duplicate shard (should be no-op)
	ring.AddShard("shard-0")

	allShards := ring.GetAllShards()
	if len(allShards) != 2 {
		t.Errorf("Adding duplicate shard changed ring size")
	}
}
...
func TestHashConsistent(t *testing.T) {
	// Consistent hash should be deterministic
	hash1 := hashConsistent("test-key")
	hash2 := hashConsistent("test-key")
