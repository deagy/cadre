...
func PortClineAgentsCmd(args []string) int {
	...
	source := fs.String("source", "", "directory containing the generated agents/ and skills/ (required)")
	...
	if *source == "" {
		fmt.Fprintf(os.Stderr, "cadre port-cline-agents: --source is required\n")
		return 2
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "cadre: --source: %v\n", err)
		return 1
	}
