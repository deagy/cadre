...
func TestABumpedKernelVersionIsDetected(t *testing.T) {
	// The kernel is separately versioned: providers declare a
	// kernel_compatibility window against it, and that window moves
	// independently of the CLI's own version. A bump has to reach the release
	// job on its own, or the kernel silently stops being publishable the way
	// the CLI did.
	files := eightManifests(t, "1.0.0")
	files["VERSION"] = "0.5.0\n"
	files[kernelVersionFile] = "package kernel\n\nconst Version = \"0.13.2\"\n"
	root, first := scratchRepo(t, files)

	commitScratch(t, root, map[string]string{
		kernelVersionFile: "package kernel\n\nconst Version = \"0.14.0\"\n",
	})

	changed := detect(t, root, first)
	if !changed["kernel"] {
		t.Error("a bumped kernel.Version was not detected, so the kernel would not be released")
	}
	if changed["cli"] || changed["plugin"] {
		t.Error("bumping the kernel reported the CLI or plugin as changed too")
	}
}

func TestAnEditToTheKernelThatIsNotAVersionBumpIsNotARelease(t *testing.T) {
	// The gate watches a source file, not a marker, so it has to read the
	// constant rather than notice the file changed at all -- otherwise every
	// edit to provider.go would cut a kernel release.
	files := eightManifests(t, "1.0.0")
	files["VERSION"] = "0.5.0\n"
	files[kernelVersionFile] = "package kernel\n\nconst Version = \"0.13.2\"\n"
	root, first := scratchRepo(t, files)
	...
	commitScratch(t, root, map[string]string{
		kernelVersionFile: "package kernel\n\n// a comment nobody releases for\nconst Version = \"0.13.2\"\n",
	})

	if detect(t, root, first)["kernel"] {
		t.Error("an edit that left the version alone was reported as a kernel release")
	}
}

func TestTheKernelVersionFileTheGateWatchesActuallyDeclaresIt(t *testing.T) {
	// The path and the pattern are both written down here, so either can drift
	// from the source. This is the same failure that switched the CLI release
	// off: a watched path that stopped being where the version lives.
	root := filepath.Dir(filepath.Dir(mustGetwd(t)))
	contents, err := os.ReadFile(filepath.Join(root, filepath.FromSlash(kernelVersionFile)))
	if err != nil {
		t.Fatalf("the gate watches %s, which does not exist: %v", kernelVersionFile, err)
	}
	found := kernelVersionLiteral.FindSubmatch(contents)
	if found == nil {
		t.Fatalf("%s no longer declares `const Version = \"...\"`, so the kernel's "+
			"version is invisible to the release gate", kernelVersionFile)
	}
	// And it must be the version the kernel actually reports. Asked of the
	// binary rather than read from the package: internal/cli is a roster-side
	// package and may not link internal/kernel -- importing it here compiles,
	// looks like a tidier assertion, and dissolves the boundary
	// TestNoRosterSidePackageImportsTheKernel exists to hold. It caught this
	// exact import while I was writing this test.
	declared := string(found[1])
	build := exec.Command("go", "run", "./cmd/agentic-sdlc", "--version")
	build.Dir = root
	reported, err := build.Output()
	if err != nil {
		t.Skipf("cannot ask the kernel its version: %v", err)
	}
	if got := strings.TrimSpace(string(reported)); got != declared {
