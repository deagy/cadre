...
| `cmd/`, `internal/` | The Go CLI (`ADR-001-CLI-GO-REFACTOR.md`'s Python-to-Go refactor of `bin/cadre`), with a repository-root `go.mod`/`go.sum` and `Makefile`. `bin/cadre.py` is deleted; `bin/cadre` is a shell shim that builds and execs `cmd/cadre`. Most subcommands are fully ported, including a complete settings resolver (`internal/config/`, mirroring `roster/shared/src/settings.py`'s precedence chain in full) and the knowledge store (`internal/knowledge/`). `cadre select` — the flagship command — is now Go (`internal/selector/`), gated on `roster/orchestration/test/test_select_differential.py`, which runs both implementations on the same machine and requires byte equality including `dispatch_fingerprint`. `CADRE_SELECT_IMPL=python` reverses it; the Python selector stays in the tree as that escape hatch and as the other half of the gate. See `internal/cli/select_agents.go`'s header and `REMAINING_PYTHON_SCOPE.md`. |
...
# Makefile wraps the same targets (build, test, test-race, lint, fmt, vet, tidy, cross-build)
```
