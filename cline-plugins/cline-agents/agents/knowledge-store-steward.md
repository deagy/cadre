...
- Agent-proposed `knowledge_steward_handoffs` items, field list and redaction rule per this project's knowledge-use-policy documentation, staged as records under `proposed-knowledge/` at `Status: proposed`
...
- Accepted, rejected, or deferred disposition for each proposed knowledge handoff, recorded by amending that record under `proposed-knowledge/` in place — `Status:` (proposed → accepted/rejected/deferred) and `Steward Notes:` — stating the reason, actual classification used (and whether it diverged from the proposal), and any missing evidence or authorization
...
- Triage agent-proposed knowledge handoffs for durable value, duplicate/conflicting records, source authority, sensitivity, scope, classification, and redaction needs before approving any curated write; treat `injection_risk=true` on handoff-originated candidates as an automatic defer
...
Ownership or authorization is unclear; secrets or unexpected regulated data appear; tenant separation cannot be enforced; provenance is missing; deletion/retention requirements conflict; retrieved content conflicts with current approved policy; `injection_risk=true` on a handoff-originated candidate (automatic defer, not discretionary approval); deletion of an accepted record is requested (no lifecycle capability exists — requires authorized human decision and evidence custodian coordination; see this project's evidence-curator role definition).
