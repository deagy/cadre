...
7. Run the orchestration test suite and confirm catalog definition paths exist. Adding a role also means bumping `EXPECTED_ROLE_COUNT` in the bundled repository-health test suite, the two count assertions in the bundled Cline port's own test suite, the metadata-file count in the bundled role-metadata test suite, `SOURCE_ROLE_COUNT`/`SOURCE_SKILL_COUNT` in `cline-plugins/cline-agents/test/presets.test.mts` (a TypeScript test, so the Python suites run green without it -- only CI's `cline-plugins` job catches it), and the hand-maintained "N roles" prose in `README.md`, `CLAUDE.md`, this repository's runbook, `docs/role-index.md`, `docs/capability-index.md`, `.claude-plugin/marketplace.json`, and `.agents/skills/role-discovery/SKILL.md`. Only some of those are test-enforced; the rest go stale silently.
8. Add a fixture to the bundled selector's golden-corpus fixtures for each new route. `test_corpus_covers_every_required_route_category` derives its required set from `routing.yaml` itself, so a route without a fixture fails. Every catalog role must also be referenced by some route, risk rule, or team recipe — the bundled routing-coverage test suite reports an unreferenced role as an orphan.
...
Files selected out of the repository tree (`roster/`, `.agents/skills/`,
`bin/`, `provider/`) are copied only when **git-tracked**, from `git ls-files`.
A new file that has not been `git add`ed is skipped silently, so a packaged
file edited to reference it ships pointing at something absent -- stage new
files before regenerating. The documentation paths above (`AGENTS.md`,
`CONTRIBUTING.md`, `IDENTITY.md`, `docs/`) are selected by a filesystem check
instead, so an untracked one is copied *in*; there the risk is committing the
packaged copy while leaving the source untracked. Staging first makes both
branches behave the same. See this repository's runbook §17 for the full sequence.
