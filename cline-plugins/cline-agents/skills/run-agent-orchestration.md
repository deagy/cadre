...
- knowledge-steward handoff expectations from this project's knowledge-use-policy documentation: durable decisions, findings, lessons, root causes, reusable patterns, or stale/conflicting guidance must be proposed to `knowledge-store-steward` with evidence and classification/scope metadata, or explicitly reported as `none`;
...
- `cline-plugins/cline-agents/` is a second, separate plugin that **does**
  dispatch a named role. It ports this repository's 74 catalog roles into
