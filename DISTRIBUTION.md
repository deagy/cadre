...
its delegation to Python `internal/selector`, or `knowledge`, or `config`)
...
for the platform matrix and naming helpers; its test pins that matrix against
`Makefile`'s `cross-build` recipe, this document, and `release.yml`'s own build
matrix, so the four cannot silently drift apart. It does not yet assert anything
...
`internal/release/platforms_test.go` pins that every platform line
does so.
...
- `plugin/bin/cadre` -- the packaged plugin's dispatcher (generated by
  `internal/generators/plugin_generation.go`; unchanged by this
  task pending the path-resolution blocker above).
- `internal/release/platforms.go` -- the platform matrix and naming helpers
  (single source of truth; a normal package, so a consumer never has to
  import a test file to reach it).
- `internal/release/platforms_test.go` -- the asset-naming/platform-matrix
  guard, checked against the Makefile, this document, and `release.yml`.
