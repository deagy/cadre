...
3. **Regenerate derived output:** Run `cadre generate-role-metadata` after any frontmatter/catalog-order change, then `cadre generate-plugin --output plugin` (and `port_cline_agents.py` when a role/skill changed) so the committed generated half of `plugin/` and `provider/` stay in sync with the source.
