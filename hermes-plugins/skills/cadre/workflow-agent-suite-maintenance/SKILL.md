---
name: workflow-agent-suite-maintenance
description: "run the cadre agent-suite-maintenance workflow."
version: 0.1.0
...
4. **Test-engineer / code-reviewer:** Rerun `go test ./...`, confirm the repository-health guards (drift, role count, workflow/schema parity) is green, and add or update selector tests and golden-corpus fixtures for any new or changed route.
...
7. **Escalation:** Stop for anything that would move gate-approval, risk-acceptance, or compliance-applicability logic between `roster/` and the kernel repository, or otherwise touch the kernel ownership boundary (`internal/orchestration/roster_boundary_test.go`) — that requires the accountable human/reviewer pairing `orchestration/escalation-policy.md` describes, not routine maintenance.

Completion requires the catalog/routing/policy edit, a clean `cadre generate-role-metadata --check` and `cadre generate-plugin --output plugin --check`, passing `go test ./...`, and an independent-review handoff tied to the exact revision.
