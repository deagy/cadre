...
- `roster/orchestration/src/routing_health.py` (`python3 roster/orchestration/src/routing_health.py`) answers "is every catalog agent reachable from routing.yaml, and does every routing.yaml agent reference resolve to a real catalog agent" (reachability/orphan/dangling-reference coverage), assuming both files already parsed and are well-typed. It also answers one question internal to routing.yaml: does any rule's `exclude_paths` fully shadow one of its own `paths` globs, leaving that glob dead while the rule keeps its `reviewers`/`human_gate` and quietly matches on keywords alone. That check is one-directional by design — it reports a glob only when every probe path the glob itself matches is excluded, so an unrepresentative probe set costs a missed finding rather than a false accusation.
