...
1. Do not assume `unclassified` means low-risk or low-priority — it is a statement about workflow-shape recognition, not about the task's actual impact. Read `matched_routes`, `matched_risks`, and the selected `agents` groups to understand what was actually matched, and `matched_route_reasons` to see *why* each route matched — the keywords, keyword groups, and path patterns that fired. An unrecognized combination is often an unexpected route match, and that field names the trigger without a read of `routing.yaml`.
