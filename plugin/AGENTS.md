...
This directory packages two systems as **4 separate, independently-installable plugins**: `cadre` (role selection, this directory's root — the only one most projects need) plus 3 optional lifecycle-governance plugins, `cadre-lifecycle-core` (`plugins/lifecycle/`), `cadre-lifecycle-github` (`plugins/lifecycle-github/`), and `cadre-lifecycle-gitlab` (`plugins/lifecycle-gitlab/`) — each self-sufficient and installable in any combination, none requiring another.

- **Cadre** (role selection, plugin `cadre`): 159 specialist roles and 20 non-authoring context packs, routing rules, orchestration runtime. The source of truth for role definitions is `roster/` at the repository root, in this same monorepo. Assets here (`agent-catalog.json`, `profiles/`, `extensions/`, `skills/`, `agents/`, `bin/cadre`, and `plugins/lifecycle/skills/`) are generated from it by `cadre generate-plugin --output plugin`.
- **Agentic SDLC** (lifecycle governance, plugins `cadre-lifecycle-core`/`-github`/`-gitlab`): G1–G10 gate schemas, run-record validation, gate authority. Owned by the lifecycle kernel, the separate repository `deagy/cadre-kernel` — a permanently separate, independently versioned and released component (`kernel-v*` tags), installed as a standalone `agentic-sdlc` binary and invoked via `bin/cadre sdlc`, never vendored into this package. `plugins/lifecycle-github/` and `plugins/lifecycle-gitlab/` are entirely hand-authored here (including their own bundled onboarding/gate-review/pending-gates-briefing skill copies and kernel bootstrap script — hand-maintained duplicates of `cadre-lifecycle-core`'s generated equivalents, kept in sync by `tools/test_plugin_duplication_health.py`, which compares the copies section by section after normalizing forge-specific vocabulary and fails on any unexplained difference); `roster/` has no concept of them.
- **Cline plugin** (`../cline-plugins/cline/`): hand-authored TypeScript plugin exposing the `agents_select` tool call, belonging to the `cadre` plugin.
- **Cline Agents plugin** (`../cline-plugins/cline-agents/`): a distinct TypeScript plugin that ports the 159 Cadre catalog roles into Cline SDK agent presets a Cline session can dispatch as background subagents. Its `index.ts`/`package.json`/`test/`/`README.md` are hand-authored; `agents/*.md` (159 roles) and `skills/*.md` (8 skills) are generated from this repository's own `agents/*.md`/`skills/*/SKILL.md` via `tools/port_cline_agents.py` (run as `--root cline-plugins --source plugin`), wired into the same regeneration pipeline as everything else here.
- **Cline Lifecycle plugin** (`../cline-plugins/cline-lifecycle/`): a third, hand-authored TypeScript plugin exposing G1–G10 Agentic SDLC lifecycle governance as 21 tool calls, each wrapping the exact `bin/cadre sdlc <subcommand>` invocation the lifecycle plugins' skills already document for Claude Code/Codex. Requires the external `agentic-sdlc` kernel to be installed separately. See `../cline-plugins/cline-lifecycle/README.md` for the tool-by-tool breakdown and CHANGELOG.md for the kernel-version-compatibility history.
...
```sh
cd ../cline-plugins/cline && npm test
cd ../cline-plugins/cline && npm run typecheck
```
...
cd ../cline-plugins/cline-agents && npm run typecheck
```

For the Cline Lifecycle plugin:

```sh
cd ../cline-plugins/cline-lifecycle && npm test
cd ../cline-plugins/cline-lifecycle && npm run typecheck
```
...
The Agentic SDLC kernel owns lifecycle gate schemas, run-record validation, and gate-authority semantics. It is consumed here as a separately installed dependency (the `agentic-sdlc` CLI released from `deagy/cadre-kernel` at the repository root and released on its own `kernel-v*` tags, invoked via `bin/cadre sdlc`), not vendored into this package.
